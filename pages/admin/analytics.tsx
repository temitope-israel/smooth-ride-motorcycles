...
// Define an interface for your data for better Type Safety
...
export default function AnalyticsPage() {
  ...
  // Helper function to create labels for Pie Charts
  const renderCustomLabel = ({ name, percent = 0 }: { name: string; percent?: number }) => {
  ...
  const getRimTypePreferences = () => {
    ...
    data.forEach((entry) => {
      ...
      if (rim) counts[rim] = (counts[rim] || 0) + 1;
    });
    return Object.entries(counts).map(([name, value]) => ({ name, value }));
  };
  ...
  const getUsageDistribution = () => {
    ...
    data.forEach((entry) => {
      ...
      if (usage) counts[usage] = (counts[usage] || 0) + 1;
    });
    return Object.entries(counts).map(([name, value]) => ({ name, value }));
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white p-4 rounded shadow">
                ...
                <p className="text-base text-center mt-2 text-gray-500">
                  📈 Peak Registration: <strong>{peakMonth.month}</strong> ({peakMonth.count})
                </p>
              </div>

              {/* Top Models */}
              <div className="bg-white p-4 rounded shadow">
                <h2 className="text-base font-semibold mb-2">🏍 Top Bike Models</h2>
                <ResponsiveContainer width="100%" height={300}>
              ...
              <div className="bg-white p-4 rounded shadow">
                <h2 className="text-base font-semibold mb-2">
                  🚦 Usage Distribution{" "}
                  <span className="text-gray-500 text-sm">(Private or Commercial Use)</span>
                </h2>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart className="text-sm">
                    <Pie
              ...
              {/* State Distribution */}
              <div className="bg-white p-4 rounded shadow">
                <h2 className="text-base font-semibold mb-2">📍 Registrations by State</h2>
                <ResponsiveContainer width="100%" height={300}>
              ...
              <div className="bg-white p-4 rounded shadow">
                <h2 className="text-base font-semibold mb-2">
                  🎨 Color Preferences{" "}
                  <span className="text-gray-500 text-sm">(Blue/Black/Red/Grey/White)</span>
                </h2>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart className="text-sm">
                    <Pie
                      ...
                      label={renderCustomLabel}
                    >
                      {getColorPreferences().map((_, index) => (
                        <Cell key={index} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Pie>
                    <Legend />
              ...
              <div className="bg-white p-4 rounded shadow">
                <h2 className="text-base font-semibold mb-2">
                  🔋 Start Type Preferences{" "}
                  <span className="text-gray-500 text-sm">(Kick-Start / Self-Start)</span>
                </h2>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart className="text-sm">
                    <Pie
                      ...
                      label={renderCustomLabel}
                    >
                      {getStartTypePreferences().map((_, index) => (
                        <Cell key={index} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Pie>
                    <Legend />
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart className="text-sm">
                    <Pie
                      ...
                      label={renderCustomLabel}
                    >
                      {getRimTypePreferences().map((_, index) => (
                        <Cell key={index} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Pie>
                    <Legend />
